...
class MemoryStream: 
  ...
  def retrieve(self, focal_points: List[str], time_step: int, 
    ...
    ### modified function to write retrieved top 10 memories to Json ###
    
    # new_ret = dict()
    # for key, val in retrieved.items(): 
    #   key = key.split("\n")[0].split(": ")[1]
    #   new_ret[key] = [i.content for i in val]
    # append_to_json("retrieved_memories.json", new_ret)
...
def extract_recency(seq_nodes: List[ConceptNode]) -> Dict[int, float]:
  ...
  """
  out = dict()
  most_recent_timestamp = max(seq_nodes, key=lambda node: node.last_retrieved).last_retrieved
  recency_decay = 0.99
    
  for node in seq_nodes:
      node_id = node.node_id
      last_retrieval_time = node.last_retrieved
      
      # Calculate recency score: recency_decay ** (max_timestep - last_retrieval_time)
      recency_score = recency_decay ** (most_recent_timestamp - last_retrieval_time)
      out[node_id] = recency_score
...
def extract_relevance(seq_nodes: List[ConceptNode], 
  ...
  # calculate cos sim for each node content      
  for node in seq_nodes:
    node_id = node.node_id
    node_content = node.content
    node_embedding = embeddings[node_content]
    
    cos_sim_score = cos_sim(node_embedding, focal_pt_embedding)
    out[node_id] = cos_sim_score
...
def run_gpt_generate_importance(
  ...
  def _func_clean_up(gpt_response, prompt=""): 
    ...
    #print("gpt_response in func_clean_up", gpt_response)
